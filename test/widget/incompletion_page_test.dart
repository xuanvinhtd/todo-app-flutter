...
void main() {
  ...
  group('Testing InCompletion Page', () {
    testWidgets('Create success a new todo to test InCompletion page', (tester) async {
      app.main();
    ...
    testWidgets('Move to incompletion page and check number of todo list', (tester) async {
      app.main();
      await tester.pumpAndSettle(const Duration(seconds: 2));
      ...
      // Tab tab button
      final Finder addButton = find.text('in_complete_title'.tr);
      await tester.tap(addButton);

      await tester.pumpAndSettle();

      // Check Open incompletion page
      expect(find.text("all_incomplete".tr), findsOneWidget);

      // Check page is empty
      expect(find.text("task_in_complete_empty".tr), hasComplete ? findsNothing : findsOneWidget);
    });
    ...
    testWidgets('Update success a todo', (tester) async {
      ...
      var textField = find.byKey(AppWidgetKeys.dialogTextField);
      const taskTitle = 'Update a task at incompletion page';
      await tester.enterText(textField, taskTitle);
    ...
    testWidgets('Update complete status for a todo', (tester) async {
      ...
      expect(firstItem.isComplete, false);
      ...
      expect(updatedItem.isComplete, true);
    });
    ...
    testWidgets('Delete success a todo', (tester) async {
      ...
      //Count data
      var inCompleteItems = HomeController.to.todos.where((e) => !e.isComplete).toList();
      var hasComplete = inCompleteItems.isNotEmpty;
      if (!hasComplete) return;
      ...
      // Touch the first item in listview
      final Finder firstCell = find.byKey(Key('todoCell${firstItem.id}'));

      await tester.drag(firstCell, const Offset(500.0, 0.0));

      await tester.pumpAndSettle();

      // Check item already deleted
